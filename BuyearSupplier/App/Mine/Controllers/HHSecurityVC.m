...
#import "HHSecurityVC.h"
#import "HHTextfieldcell.h"
//#import "HXEditPwdAPI.h"

@interface HHSecurityVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
- (void)viewDidLoad {
    ...
    self.leftTitleArr = @[@"当前安全密码",@"新安全密码",@"确认新安全密码"];
    self.placeHolderArr = @[@"点击编辑安全密码",@"点击编辑新安全密码",@"再次编辑新安全密码"];
}
- (void)saveBtnAction{
    ...
    if (!vaildStr) {
        ...
        [[[HHMineAPI ModifySecurityPasswordWithold_pwd:login_pdStr new_pwd:new_login_pdStr repeat_new_pwd:commit_new_login_pdStr] netWorkClient] postRequestInView:nil finishedBlock:^(HHLoginAPI *api, NSError *error) {
